...
# plotacpclust(USArrests)


plotacpclust = function(data,xax=1,yax=2,hcut,cor=TRUE,clustermethod="ave",colbacktitle="#e8c9c1",wcos=3,Rpowered=FALSE,...){
	...
	s.corcircle(pcl[,c(xax,yax)],1,2,sub=paste("(",xax,"-",yax,") ",round(sum(pclperc[c(xax,yax)]),0),"%",sep=""),possub="bottomright",csub=3,clabel=2)
	wsel=c(xax,yax)
	...
	#names.arg=paste("g",ngrp:1,sep="")
	mp=barplot(as.vector(rev(ttab)),horiz=TRUE,space=0,col=rev(colorsnames),xlim=c(0,max(ttab)+10),axes=FALSE,main="Groups",axisnames=FALSE)
	...
	text(x=selscores[out,1],y=selscores[out,2],labels=dimnames(selscores)[[1]][out],col=(colorsnames[groupes])[out])
	box()
...
plotdens=function(X, npts = 200, range = 1.5, xlab = "", ylab = "", main = "", ...)
{
	...
	dy <- dens$y
	dx2 <- c(dx[dx <= fquX], fquX, dx[(dx > fquX) & (dx <= medX)], medX, dx[
		(dx > medX) & (dx <= tquX)], tquX, dx[dx > tquX])
	
	dy2 <- 	c(dy[dx <= fquX], fquy, dy[(dx > fquX) & (dx <= medX)], medy, dy[(dx > medX) & (dx <= tquX)], tquy, dy[dx > tquX])
	...
	add = TRUE, col = 11., border=FALSE,colline = 1., Fill = TRUE)
{
	...
	polygon(x = c(x[1.], x[2.], x[2.], x[1.]), y = c(y[2.], y[2.], y[1.], y[
		1.]), density = 0.)
}
